  ...
  <div class="max-w-full mx-auto mt-8 p-6 bg-white shadow-md rounded-md">
    ...
    <form action="{{ route('galeri.update', $galeri->id) }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="relative z-0 w-full mb-5">
        <img 
          id="image-preview" 
          class="w-32 h-32 object-cover rounded-md {{ $galeri->gambar ? '' : 'hidden' }}" 
  ...
  <script>
    function previewImage() {
      ...
      if (file) {
        ...
        imagePreview.src = '{{ $galeri->gambar ? asset("storage/" . $galeri->gambar) : "" }}'; // Tampilkan gambar lama jika tidak ada file baru
        if (!imagePreview.src) {
